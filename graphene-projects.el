...
(defvar graphene-project-folder (concat user-emacs-directory "projects/"))
...
(defvar graphene-project-root-dir)
...
;; Set graphene-project-root-dir, set default-directory, update speedbar
(defun graphene-set-project-directory (dir)
  (setq graphene-project-root-dir dir)
  (message (concat "Set graphene-project-root-dir to " graphene-project-root-dir))
...
(add-hook 'graphene-project-before-open-hook
          (lambda ()
            (progn
              (if (boundp 'graphene-project-root-dir)
                  (project-save))
...
(add-hook 'graphene-project-after-open-hook
          (lambda ()
            (progn
              (graphene-set-project-directory
                    (project-default-directory (project-current)))
              (message (concat "Loading project desktop from " graphene-project-root-dir))
              (desktop-read)
              )))
;; Save the project desktop.
(add-hook 'project-after-save-hook
          (lambda ()
            (progn
              (message (format "Saving project desktop in %s" graphene-project-root-dir))
              (desktop-save graphene-project-root-dir)
              )))
